    ...
    <div className="min-h-screen bg-background flex">
      ...
      `}>
        ...
        <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
          <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider px-4 mb-3">Menu</p>
          {navItems.map(item => (
            <button
              key={item.id}
            ...
            <div className="flex items-center gap-3">
              <button className="relative w-10 h-10 rounded-xl bg-card border border-border flex items-center justify-center text-foreground hover:bg-muted transition-colors">
                ...
                {announcements && announcements.length > 0 && (
                  <span className="absolute -top-1 -right-1 w-5 h-5 bg-accent text-white text-[10px] font-bold rounded-full flex items-center justify-center shadow-sm">
                    {announcements.length}
                  </span>
                )}
              </button>
              <div className="hidden sm:flex items-center gap-3 pl-3 border-l border-border">
                <div className="w-9 h-9 rounded-xl bg-accent/20 text-accent flex items-center justify-center font-bold text-sm">ST</div>
                ...
                <div className="p-6 space-y-5">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1 flex items-center gap-1.5">
                      <MapPin size={14} /> {orderModal.seller} • {orderModal.region}
                    </p>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      />
                      <button
                        onClick={() => setOrderQuantity((parseInt(orderQuantity) + 1).toString())}
                        className="w-11 h-11 rounded-xl border border-border flex items-center justify-center hover:bg-muted transition-colors font-bold text-lg"
                      >+</button>
                    </div>
                  </div>